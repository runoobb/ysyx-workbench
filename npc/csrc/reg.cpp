...
bool checkregs(regfile *ref, regfile *dut) {
  if(ref->pc != dut->pc){
    ...
    printf("next reg pc is diff: ref = 0x%08x, dut = 0x%08x\n",ref->pc,dut->pc);
    return false;
  }
  for (int i = 0; i < ARRLEN(regs); i++) {
    if(ref->x[i] != dut->x[i]){
      printf("difftest error at nextpc = 0x%08x, ",dut->pc);
      printf("reg %s is diff: ref = 0x%08x, dut = 0x%08x\n",regs[i],ref->x[i],dut->x[i]);
      return false;
    }
  }
