...
#define INSTPAT_MATCH(s, name, type, ... /* execute body */ ) { \
  ...
  INSTPAT("??????? ????? ????? 001 ????? 11100 11", csrrw  , I, R(rd) = SR(val_imm); SR(val_imm) = src1);
  INSTPAT("??????? ????? ????? 010 ????? 11100 11", csrrs  , I, R(rd) = SR(val_imm); SR(val_imm) = SR(val_imm) | src1);
  INSTPAT("??????? ????? ????? 011 ????? 11100 11", csrrc  , I, R(rd) = SR(val_imm); SR(val_imm) = SR(val_imm) & ~src1);
  INSTPAT("0000000 00001 00000 000 00000 11100 11", ebreak , N, NEMUTRAP(s->pc, R(10))); // R(10) is $a0
  INSTPAT("0000000 00000 00000 000 00000 11100 11", ecall  , N, s->dnpc = isa_raise_intr(R(17), s->dnpc)); //R(17) is $a7
  INSTPAT("0011000 00010 00000 000 00000 11100 11", mret   , N, s->dnpc = SR(2)); //SR(2) is $mepc
